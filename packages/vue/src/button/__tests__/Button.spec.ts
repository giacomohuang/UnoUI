...
describe('button', () => {
  ...
  it('keeps outline and mono hover colors clear without darkening', () => {
    ...
    expect(outline).toContain('after:text-red-500')
    expect(mono).toContain('after:text-red-500')
  ...
  it('keeps dashed and link color classes distinct', () => {
    ...
    expect(link).not.toContain('shadow-[0_0_0_6px_currentColor]')
    expect(link).not.toContain('after:text-orange-500')
  })
})
